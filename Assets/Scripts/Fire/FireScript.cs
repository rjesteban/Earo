...
public class FireScript : MonoBehaviour {
	...
	public void match(){
		...
		if(expected.Equals(answer)){
			...
			//targetFire.GetComponent<FireAblaze>().turnOffFire(targetFire);
			//ExtinguishFire.targetFire.GetComponent<FireAblaze>().turnOffFire();
			Destroy(ExtinguishFire.instance.targetFire);
			Destroy(gameObject);
		}
		else
			Destroy(gameObject);
